...
export default function ContactMailto() {
  ...
  const onSubmit = (data: ContactFormData) => {
    setSending(true);
    try {
      const mailtoLink = `mailto:roshanaryal.dev@gmail.com?subject=${encodeURIComponent(data.subject)}&body=${encodeURIComponent(`Name: ${data.name}\nEmail: ${data.email}\n\n${data.message}`)}`;
      ...
      toast.error("> CONNECTION ERROR", {
        description: "Failed to open email client. Try again later.",
      });
    } finally {
      setSending(false);
    }
